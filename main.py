...
class Game:
    def __init__(self):
        ...
        self.screenshake = 0  # ← 修正拼字錯誤（原本是 screenshske）
    ...
    async def run(self):  # ← 改成 async
        # 顯示啟動畫面
        self.display.fill((0, 0, 0))
        draw_text(self.display, "NinjaGame", 48, 160, 20)
        ...
        draw_text(self.display, "Press any key to start the game...", 12, 160, 220)
        ...
        while True:
            ...
            if not len(self.enemies):
                ...
                if self.transition > 30:
                    ...
                    MAX_LEVEL = 4  # 你有幾個關卡就填幾
                    self.level = min(self.level + 1, MAX_LEVEL - 1)
                    self.load_level(self.level)
            ...
            pygame.display.update()
            self.clock.tick(60)
            await asyncio.sleep(0)  # ← 關鍵！每幀讓出控制權
